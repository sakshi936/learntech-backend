...
const roadmapStructureCache:any = {};
 ...
 export const getRecommendations = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    const progressRecords = await Progress.find(
      { userId },
      { roadmapSlug: 1, completedItems: 1, percentage: 1, lastUpdated: 1 }
    ).sort({ lastUpdated: -1 }).lean(); // Use lean() for faster queries
    ...
    const roadmapTitles = roadmaps.reduce((map:any, r) => {
      map[r.slug] = r.title;
      return map;
    }, {});
      ...
      if (activeRoadmaps.length > 0) {
        ...
        if (roadmapStructureCache[roadmapSlug] && 
          ...
          if (fs.existsSync(filePath)) {
            structureData = JSON.parse(fs.readFileSync(filePath, 'utf8'));
            // Store in cache
        ...
        const flatCompletedItems: any[] | undefined = [];
        const flattener = (obj:any, parentPath:any = [], result:any = []) => {
    ...
    try {
      const result:any = await Promise.race([recommendationPromise, timeoutPromise]);
